...
class AddArtifactParams(Params):
    name: str = Param(
        description="Name of artifact", required=False, default="User created artifact"
    )
    container_id: int = Param(
        description="Container to add the artifact to",
        required=False,
        cef_types=["phantom container id"],
    )
    label: str = Param(description="Artifact label", required=False, default="event")
    source_data_identifier: str = Param(
        description="Source data identifier", required=True
    )
    cef_name: str = Param(description="Name of a CEF field", required=False)
    cef_value: str = Param(
        description="Value for the CEF field", required=False, cef_types=["*"]
    )
    cef_dictionary: str = Param(
        description="JSON string of CEF fields and values", required=False
    )
    contains: str = Param(description="Data type for the CEF field", required=False)
    run_automation: bool = Param(
        description="Run active playbooks", required=False, default=False
    )
    determine_contains: bool = Param(
        description="Determine the contains for the CEF fields",
        required=False,
        default=True,
    )
...
def add_artifact(
    params: AddArtifactParams, soar: SOARClient, asset: Asset
) -> list[AddArtifactOutput]:
        ...
        try:
            ...
            if not isinstance(loaded_cef, dict):
                raise PhantomClientError(
                    "Please provide cef_dictionary parameter in JSON format"
                )
        except json.JSONDecodeError as e:
            raise PhantomClientError(
                f"Could not load JSON from CEF parameter: {e}"
            ) from e
    ...
    if params.contains:
        try:
            ...
            if isinstance(loaded_contains, list):
                raise PhantomClientError(
                    "Please provide contains parameter in JSON or string format only"
                )
            ...
            if params.cef_name and params.cef_value:
                contains_list = list(
                    filter(None, [x.strip() for x in params.contains.split(",")])
                )
                loaded_contains[params.cef_name] = contains_list
            else:
                raise PhantomClientError(
                    "Please provide contains parameter in JSON format"
                ) from e
    ...
    if params.determine_contains:
        for cef_name in loaded_cef:
            ...
            if cef_name not in CEF_NAME_MAPPING:
                determined = (
                    determine_contains(loaded_cef[cef_name])
                    if loaded_cef[cef_name]
                    else None
                )
                if determined:
    ...
    try:
        _response, resp_data = client.make_rest_call(
            "/rest/artifact", method="post", data=artifact
        )
